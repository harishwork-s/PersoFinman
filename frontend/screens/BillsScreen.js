...
export default function BillsScreen({ t, language, setLanguage, onProfilePress }) {
  ...
  async function loadItems() {
    ...
    setError("");
    try {
      const saved = await loadCollection(STORAGE_KEYS.bills);
      setItems(sortByDate(saved.map(normalizeBill), "dueDate"));
    } catch (err) {
  ...
  async function saveItems(nextItems) {
    ...
    setItems(sorted);
    await saveCollection(STORAGE_KEYS.bills, sorted);
  }
  ...
  async function addOrUpdateItem() {
    ...
    try {
      ...
      if (editId) {
        ...
        item.notificationIds = await safeSchedule("bill", item);
